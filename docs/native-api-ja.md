...
- **`status`** — 全体状態 `building` / `ready` / `error`。抽出状態は `source_status`、索引状態は `index_status`、除外状態は `retrieval_status` に分けて示します。
...
ここでの `status` は保存済み抽出状態（`pending` / `extracted` / `error`）です。
全体状態と source identity は一覧エンドポイントで確認してください：
